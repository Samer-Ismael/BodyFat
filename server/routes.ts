...
import { DEMO_DATES, DEMO_ROWS } from "./demo.js";
...
function userExists(id: number): boolean {
  initDb();
  return db.prepare("SELECT 1 FROM users WHERE id = ?").get(id) != null;
}
...
function loadSettings(userId: number) {
  initDb();
  const raw = db
    .prepare(
      `SELECT height_cm, goal_body_fat, goal_waist_cm, goal_weight_kg, bf_method, birth_date
...
apiRouter.put("/settings", (req, res) => {
  ...
  db.prepare(
    `UPDATE user_settings SET height_cm = ?, goal_body_fat = ?, goal_waist_cm = ?, goal_weight_kg = ?, bf_method = ?, birth_date = ?
     WHERE user_id = ?`
  ).run(height_cm, goal_body_fat, goal_waist_cm, goal_weight_kg, bf_method, birth_date, userId);
  res.json(loadSettings(userId));
});
  ...
  const out = rawRows
    ...
    .filter(
      (row) =>
        ...
        Number.isFinite(row.waist_cm) &&
        Number.isFinite(row.neck_cm) &&
        Number.isFinite(row.id) &&
        row.entry_date.length > 0
    );
  res.json(out);
});
